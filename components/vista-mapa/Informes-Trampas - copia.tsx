...
export function InformesTrampas({
  ...
  // IDs
  const [trampaNumeroSeleccionado, setTrampaNumeroSeleccionado] = useState<number | ''>(
    informeExistente?.trampaNumero ?? ''
  )
  const [errorTrampa, setErrorTrampa] = useState<string>('')
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (onGuardado) onGuardado(data.informe)
      }
    } catch (err) {
