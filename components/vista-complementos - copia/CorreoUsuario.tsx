...
export default function CorreoUsuario({
  ...
  const handleGuardarCorreo = async () => {
    ...
    try {
      const res = await fetch('/api/mail', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ usuario, nuevoCorreo: nuevoCorreo.trim() }),
      })
  ...
  const handleEliminarCorreo = async () => {
    try {
      const res = await fetch('/api/mail', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (result.success) {
        setCorreo(null)
        setMensaje('🗑️ Correo eliminado correctamente.')
      } else {
        setMensaje(`❌ Error: ${result.error || 'No se pudo eliminar el correo.'}`)
      }
    } catch (err) {
      console.error('❌ Error en eliminar correo:', err)
      setMensaje('❌ Error al conectar con el servidor.')
    }
  }
      ...
      {editando ? (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="destructive" onClick={handleEliminarCorreo}>
